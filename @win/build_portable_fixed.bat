...
:: Copy Qt6 QML runtime modules (CRITICAL for QML functionality!)
echo %BLUE%[INFO]%NC% Copying Qt6 QML runtime modules...
if not exist "qml\QtQuick" mkdir "qml\QtQuick"
if not exist "qml\QtQml" mkdir "qml\QtQml"
if not exist "qml\Qt" mkdir "qml\Qt"

:: Copy essential QML modules
xcopy "C:\msys64\mingw64\share\qt6\qml\QtQuick" "qml\QtQuick\" /E /I /Y >nul 2>&1
xcopy "C:\msys64\mingw64\share\qt6\qml\QtQml" "qml\QtQml\" /E /I /Y >nul 2>&1
xcopy "C:\msys64\mingw64\share\qt6\qml\Qt" "qml\Qt\" /E /I /Y >nul 2>&1
xcopy "C:\msys64\mingw64\share\qt6\qml\QtCore" "qml\QtCore\" /E /I /Y >nul 2>&1
xcopy "C:\msys64\mingw64\share\qt6\qml\QtNetwork" "qml\QtNetwork\" /E /I /Y >nul 2>&1

:: Copy QML type information files
copy "C:\msys64\mingw64\share\qt6\qml\builtins.qmltypes" "qml\" >nul 2>&1
copy "C:\msys64\mingw64\share\qt6\qml\jsroot.qmltypes" "qml\" >nul 2>&1
...
:: Qt6 dependencies (CRITICAL - these were missing!)
echo %BLUE%[INFO]%NC% Copying Qt6 dependencies...
copy "%MSYS2_PATH%\libb2-1.dll" . >nul 2>&1
copy "%MSYS2_PATH%\libdouble-conversion.dll" . >nul 2>&1
copy "%MSYS2_PATH%\libicuin77.dll" . >nul 2>&1
copy "%MSYS2_PATH%\libicuuc77.dll" . >nul 2>&1
copy "%MSYS2_PATH%\libicudt77.dll" . >nul 2>&1
copy "%MSYS2_PATH%\libpcre2-16-0.dll" . >nul 2>&1
copy "%MSYS2_PATH%\libmd4c.dll" . >nul 2>&1
...
:: Qt6 QML tooling plugins (CRITICAL for QML debugging and runtime!)
echo %BLUE%[INFO]%NC% Copying Qt6 QML tooling plugins...
if not exist "qmltooling" mkdir "qmltooling"
copy "C:\msys64\mingw64\share\qt6\plugins\qmltooling\*.dll" "qmltooling\" >nul 2>&1

:: Qt6 generic plugins
if not exist "generic" mkdir "generic"
copy "C:\msys64\mingw64\share\qt6\plugins\generic\*.dll" "generic\" >nul 2>&1

:: Qt6 styles plugins
if not exist "styles" mkdir "styles"
copy "C:\msys64\mingw64\share\qt6\plugins\styles\*.dll" "styles\" >nul 2>&1

:: Qt6 TLS plugins
if not exist "tls" mkdir "tls"
copy "C:\msys64\mingw64\share\qt6\plugins\tls\*.dll" "tls\" >nul 2>&1
...
:: Create qt.conf for portable deployment (CRITICAL!)
echo %BLUE%[INFO]%NC% Creating qt.conf for portable deployment...
echo [Paths] > qt.conf
...
echo. >> "Player-by-HEIMLICH-Portable.bat"
echo :: Set Qt plugin paths for portable deployment >> "Player-by-HEIMLICH-Portable.bat"
echo set QT_PLUGIN_PATH=%%~dp0 >> "Player-by-HEIMLICH-Portable.bat"
echo set QML_IMPORT_PATH=%%~dp0qml >> "Player-by-HEIMLICH-Portable.bat"
echo set QML2_IMPORT_PATH=%%~dp0qml >> "Player-by-HEIMLICH-Portable.bat"
echo set QT_QPA_PLATFORM_PLUGIN_PATH=%%~dp0platforms >> "Player-by-HEIMLICH-Portable.bat"
echo set QT_LOGGING_RULES=qt.qml.debug=true >> "Player-by-HEIMLICH-Portable.bat"
echo. >> "Player-by-HEIMLICH-Portable.bat"
echo echo Starting Player by HEIMLICH® Portable... >> "Player-by-HEIMLICH-Portable.bat"
...
"%MSYS2_PATH%\objdump.exe" -p Player-by-HEIMLICH.exe | findstr "DLL Name:" > final_deps.txt
